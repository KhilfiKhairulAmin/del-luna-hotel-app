    ...
    @FXML private void handleSignUp(ActionEvent event) {
        ...
        if (isValid) {
            try {
            	...
            	guests.removeIf(guest -> !guest.email.equals(emailField.getText()));
            	...
            	if (guests.size() == 0) {  // No one has the same email as the entered one
                    Guest newGuest = new Guest(
                            ...
                            passwordField.getText(), // Password hash will be set
                            securityAnswerField.getText()  // Security answer hash will be set
                        );
